...
For example, if we know that `Lisa` is the daughter of `Homer` and we create a logical rule that all daughters identify as `gender:woman` and that all `daughters: age` are necessarily less than `father:age` then we can reasonably infer some things about Lisa: We know that `Lisa` is a `woman` and that, by being the daughter of `Homer` she is younger in age. And, we know this without having to explicitly state that `Lisa is a woman` - our computers can simply reason about this relations through the logical statement that "All daughters identify as women, and Lisa is a daughter"
